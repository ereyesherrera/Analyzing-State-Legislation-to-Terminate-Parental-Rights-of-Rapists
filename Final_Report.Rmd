---
title: "Analyzing State Legislation to Terminate Parental Rights of Rapists"
author: 
...
```{r data}
data <- read_csv("files/data/Honors_stats_updated_nov18_2.csv")
data <- data %>%
  select(-X22) %>%
...
* **Cluster 1**: States whose representation in the state legislature is Republican favored and has "medium" representation of women. It is a tier below Cluster 3 and a tier higher than Cluster 3 in terms of "equality" for women and has the strongest average law strength. We can see from the map that these states are scattered across the US, with concentrations in the southeast and Midwest. This is surprising because these states have the highest average law strength, yet their representation and resources for women could be better.

* **Cluster 2**: States whose representation in the state legislature is generally split and has the worst resources, "equality", and representation for women, with an average law strength of 2 (rape conviction). These states are spread throughout the lower middle and lower east of the country. We would expect states that do not have great favorability towards women to have low average law strengths, which is exactly what we see here.

* **Cluster 3**: States whose representation in the state legislature is strongly favored towards Democratic and the most representation of women. These states have the most resources for women and the best "equality", with an average law strength of about 3 (ambiguous burden of proof). These states are mainly concentrated on the west coast and northeast. This is also surprising because we would expect those states that have the most representation and favorability towards women to have the best law strength, but that is not the case.
...
# Partisanship as a Predictor for Law Strength
...
Moreover, six (40%) “blue” (majority democratic legislators in the state legislature), 2 (13.3%) “purple” (split between democratic and republican legislators in the state legislature) and 7 (46.6%) “red” (majority republican legislators in the state legislature) states use the clear and convincing standard. There seems to be no significant variation along party lines with regard to a higher burden of proof.
...
data %>%
  ...
  scale_color_brewer(palette = "Dark2") + transition_states(year_passage)+ shadow_trail()
```
...
There are seven states that have amended their legislation since initial passage, Colorado, Connecticut, Hawaii,  Illinois, Missouri, Pennsylvania, and Texas. 
...
Colorado first passed legislation requiring a rape conviction in 2013, with a quick turnaround in 2014 of allowing for clear and convincing evidence to suffice. Connecticut on the other hand, took 15 years (2001-2016) to amend their legislation. Hawaii passed legislation in 2013, but amended in 2016 to forgo the requirement of a rape conviction in favor of the lower burden of proof, clear and convincing evidence. Illinois' amendment in 2016, after the bill's passage in 2013, neither strengthened nor weakened the law, but presented an ambiguous burden of proof. Missouri amended the law in 2016 to clear and convincing evidence, though there remains ambiguity, with an initial passage in 1997. Pennsylvania was the earliest of all states to pass legislation in 1992, with an unspecified burden of proof, and when the state amended in 2015, the burden of proof was increased to a rape conviction. Texas followed the same trajectory, passing initial legislation in 1997, and amending in 2017.

```{r}
# Looking only at states that amended law
amend_passed <- data %>%
  replace_na(list(year_amend = "NA")) %>%
  filter(year_amend != "NA") %>%
  select(year_amend, year_passage, State, law_strength) %>%
  mutate(year_amend = as.integer(year_amend), year_passage = as.integer(year_passage)) %>%
  pivot_longer(cols = c("year_amend", "year_passage"), names_to = "Type", values_to = "Year") %>%
  mutate(Type = factor(Type)) 

# Renaming levels
levels(amend_passed$Type) <- list(Amended = "year_amend", `First Passed` = "year_passage")

# Visualization
amend_passed %>%
  ggplot(aes(x = State, y = Year, color = Type)) +
  geom_point() +
  theme_minimal() +
  geom_label_repel(aes(label = Year), seed = 112, point.padding = 0.1, size = 2) +
  labs(title = "States With Amended Laws", 
       caption = "Note: Parentheses indicate the law strength a state changed to when law amended") +
  theme(plot.title = element_text(face = "bold"),
        plot.caption = element_text(face = "italic")) +
  scale_x_discrete(labels = c("Colorado \n (5)", "Connecticut \n (5)", "Hawaii \n (5)", 
                        "Illinois \n (3)", "Missouri \n (2)", "Pennsylvania \n (2)",
                        "Texas \n (2)"))
  
```

Notable takeaways here are that Colorado, Connecticut, and Hawaii, all democratically controlled at the time of amendment, amended to a lower burden of proof, whereas Pennsylvania and Texas, both republican controlled, amended to a higher burden of proof.

Data visualizations representing law passage and strength over time indicate stronger and more abundant laws protecting rape victim's rights to child custody since 2010.  Though it is difficult to clearly relate the evidence of focusing events such as the CNN stories and The Rape Survivor Child Custody Act as heavily impacting the passage of laws, there is an undeniable trend of states passing landmark custody legislation after around 2010.  Regarding focus events impacting law strength, states that amended their laws after 2010 showed multiple trends of favoring a higher caliber of the law power. This could be because of the multiple CNN stories impacts on the public conscious or the legal influence of The Rape Survivor Child Custody of 2015.

# Analyzing Specific Variables

We have included a Shiny app below that allows a user to compare states they are interested in looking at in the same way we compared the three states above. To view the app in a new window, visit [this page](https://ereyesherrera.shinyapps.io/files/). It is important to note that we needed to remove the variable of `bill_name` from our original data because of an error that entries in that variable did not use UTF-8 characters. This can be remedied by saving the original data set to preserve special characters, but because it is one of the lesser important variables in our analysis, we have decided to leave it out here.
...
Equal pay rank and proportion of equal pay for each state against law strength are examined below. It is essential to note here that proportions and rankings are updated to 2018 numbers, which for some states that passed legislation in the '90, like California, Texas, Missouri and Pennsylvania, are not necessarily reflective of the climate at the time of passage. As we may have expected, the proportion of equal pay increases when the equal pay rank is low (with 1 being the best, and 51 being the worst). However, there is no distinguishable pattern between these variables and the law strength for a state. Though equal pay for women is an important metric to consider when thinking about the favorability towards women, there may be many other important variables with deeper social implications that may be changing how states rule on this legislation.
...
Another surprising finding is that the percentage of women in both chambers of a state legislature does not necessarily lead to a better law strength, especially when also considering the party at the time of legislation. We can see that this may be the case for Blue states, where there seems to be a relationship between the percentage of women in the state legislature and law strength, but the relationship is much more complicated for Red states. For Blue states specifically, we can notice that states in which have 30 or more percent of women in both state legislative chambers at the times of the bill’s passage also have a law strength of five, showing the relationship between the two variables. However, for red states, the relationship between the two variables is much more mixed. As such, we are again left to wonder other factors that are driving states to push for a better law strength.
...
All in all, analyzing specific variables in our data set as it pertains to a state's law strength reveal that there is really no distinguishable pattern in how states are deciding to rule on this problem despite some variables reflecting perfectly what we would expect in terms of representation and resources for women and the legislation in question. Though these variables represent important factors to consider when discussing women's rights and resources available to them, we observed when comparing three very different states with the same law strength that there must be an underlying driving force in how the legislation is being treated. These variables are only the beginning when understanding this issue and serve as a basis for how states need to improve.

# Conclusion

On the whole, the factors we have analyzed cannot be classified as key predictors for passing legislation to terminate rapists' parental rights. What makes predicting how legislators will vote on this particular issue so difficult is that it converges the aforementioned categories of women’s issues and parental rights in general with the unique addition of sexual assault. While looking at how states address domestic abuse in custody cases is a start, the reality is that rape is widely under-reported and even more rarely prosecuted. Because the evidence is less concrete than for other crimes that might be presented in a custody battle, such as domestic abuse, the gray area is larger. Furthermore, peoples’ attitudes toward rape vary widely based on ideology, gender, religion, and personal experience, so there are more factors to consider when trying to predict how a particular legislator will vote.

While we found some interesting correlations throughout this project, and some results that may end up being significant for research on policy work, or perhaps parental rights/sexual assault legislation more generally, our varied figures and inconsistencies indicate that a project of this scope may benefit from a more qualitative data collection. A lawyer, lobbyist, legislator, public policy actor or constituent may be able to speak more to the process leading to passage than quantitative data would show. Some future considerations include: any variables that we, as students and not experienced politicians, have not accounted for, any local, statewide, or national news stories that impacted the passage of the legislation, any constituent or interest group outreach that was unknown to the public, any focusing events that existed but seemed not to have an effect on the legislature, the degree to which the specter of false accusations hovered over the legislature and perhaps contributed to more hesitancy, as well as any noteworthy loopholes/exceptions in relevant legislation.
